...
export type SourceExcelBalanceRow = {
  account: string;
  label: string;
  balance: number;
};

export type SourceExcelBalances = {
  status: 'processing' | 'done' | 'error';
  balanceN: SourceExcelBalanceRow[];
  balanceN1: SourceExcelBalanceRow[];
  parsedAt: string;
  sourcePath: string;
  errorMessage?: string;
};
...
export interface ProjectFormData {
  ...
  source_excel_file: SourceExcelFile | null;
  source_excel_balances: SourceExcelBalances | null;
