  ...
  persist(
    (set, get) => ({
      ...
      addToRecentProjects: (project) => {
        set((state) => {
          // Remove the project if it already exists in the recent projects
          const filteredProjects = state.recentProjects.filter(p => p.id !== project.id);
          
          // Add the project to the beginning of the array
          const newRecentProjects = [project, ...filteredProjects];
          
          // Keep only the 3 most recent projects
          return { recentProjects: newRecentProjects.slice(0, 3) };
        });
      },
