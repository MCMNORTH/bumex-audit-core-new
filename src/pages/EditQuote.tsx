...
import { User, Currency, InvoiceItem, Quote } from "@/types";
import { 
  Select,
  SelectContent,
  SelectItem,
    ...
    const fetchQuote = async () => {
      ...
      try {
        ...
        if (fetchedQuote) {
          ...
          if (fetchedQuote.status !== 'draft' && fetchedQuote.status !== 'pending') {
            toast({
              title: "Cannot Edit Quote",
              description: "This quote cannot be edited because it has been accepted, rejected, or expired.",
              variant: "destructive"
            });
          ...
          // Set form values
          form.reset({
            userId: fetchedQuote.userId || "",
            clientName: fetchedQuote.clientName,
            clientContact: fetchedQuote.clientContact || "",
          ...
          setSearchTerm(fetchedQuote.clientName);
        } else {
          toast({
            title: "Error",
