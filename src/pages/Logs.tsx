      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('logs.latestActivities')}</CardDescription>
          </CardHeader>
          <CardContent>
            {filteredLogs.length > 0 ? (
                    ...
                    <AccordionContent className="px-4 pb-4">
                      <div className="space-y-3 pt-2 border-t border-gray-100">
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            <h4 className="text-sm font-medium text-gray-900 mb-2">{t('logs.logDetails')}</h4>
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                <span className="text-gray-500">{t('logs.logId')}:</span>
                                <span className="text-gray-900 font-mono text-xs">{log.id}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">{t('logs.userId')}:</span>
                                <span className="text-gray-900 font-mono text-xs">{log.user_id}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">{t('logs.targetId')}:</span>
                                <span className="text-gray-900 font-mono text-xs">{log.target_id}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">{t('logs.action')}:</span>
                                <span className="text-gray-900">{log.action}</span>
                              </div>
                            </div>
                          </div>
                          <div>
                            <h4 className="text-sm font-medium text-gray-900 mb-2">{t('logs.timestampInfo')}</h4>
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                <span className="text-gray-500">{t('logs.date')}:</span>
                                <span className="text-gray-900">{log.timestamp.toLocaleDateString()}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">{t('logs.time')}:</span>
                                <span className="text-gray-900">{log.timestamp.toLocaleTimeString()}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">{t('logs.fullTimestamp')}:</span>
                                <span className="text-gray-900 font-mono text-xs">{log.timestamp.toISOString()}</span>
                              </div>
                            </div>
                          </div>
                          <div>
                            <h4 className="text-sm font-medium text-gray-900 mb-2">{t('logs.clientInfo')}</h4>
                            <div className="space-y-3 text-sm">
                              <div className="flex items-start space-x-2">
                                <Globe className="h-3 w-3 text-gray-400 mt-0.5" />
                                <div className="flex-1">
                                  <span className="text-gray-500">{t('logs.ipAddress')}:</span>
                              ...
                              {((log as any).country || (log as any).city) && (
                                <div className="flex items-start space-x-2">
                                  ...
                                  <div className="flex-1">
                                    <span className="text-gray-500">{t('logs.location')}:</span>
                                    ...
                                    {(log as any).timezone && (
                                      <div className="text-gray-600 text-xs">
                                        {t('logs.timezone')}: {(log as any).timezone}
                                      </div>
                                    )}
                                  </div>
                                </div>
                              ...
                              {(log as any).isp && (
                                <div className="flex items-start space-x-2">
                                  ...
                                  <div className="flex-1">
                                    <span className="text-gray-500">{t('logs.isp')}:</span>
                                    <div className="text-gray-900 text-xs mt-1">
                                      {(log as any).isp}
                                    </div>
                                  </div>
                              ...
                              <div className="flex items-start space-x-2">
                                ...
                                <div className="flex-1">
                                  <span className="text-gray-500">{t('logs.userAgent')}:</span>
                                  <div className="text-gray-900 text-xs mt-1">
                                    {(log as any).user_agent ? (
                                      <div className="space-y-1">
                                        ...
                                        {(log as any).user_agent.length > 100 && (
                                          ...
                                          >
                                            {expandedUserAgents.has(log.id) ? t('logs.showLess') : t('logs.showMore')}
                                          </button>
                                        )}
                                      </div>
                                    ) : (
                        ...
                        {log.details && (
                          <div>
                            <h4 className="text-sm font-medium text-gray-900 mb-2">{t('logs.additionalDetails')}</h4>
              ...
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">{t('logs.noLogsFound')}</h3>
