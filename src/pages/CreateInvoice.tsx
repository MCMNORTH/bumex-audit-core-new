...
export default function CreateInvoice() {
  ...
  useEffect(() => {
    if (searchTerm) {
      const filtered = users.filter(user => (user.fullName || user.name || "").toLowerCase().includes(searchTerm.toLowerCase()) || (user.email || "").toLowerCase().includes(searchTerm.toLowerCase()));
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative">
                  ...
                  field
                        ...
                        <FormControl>
                          <Input placeholder="Search for a client..." value={searchTerm} onChange={handleSearchChange} onFocus={() => setShowSuggestions(true)} onBlur={() => {
                  ...
                  {showSuggestions && filteredUsers.length > 0 && <div className="absolute z-10 w-full bg-white border border-gray-200 mt-1 rounded-md max-h-60 overflow-y-auto shadow-lg">
                      {filteredUsers.map(user => <div key={user.id} className="p-2 hover:bg-gray-100 cursor-pointer" onClick={() => selectUser(user)}>
                ...
                <FormField control={form.control} name="clientContact" render={({
                field
              }) => <FormItem>
                      <FormLabel>Client Contact (Email/Phone)</FormLabel>
                      <FormControl>
                        <Input placeholder="client@example.com or +123456789" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>} />
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {fields.map((field, index) => <div key={field.id} className="grid grid-cols-12 gap-4 items-end border p-4 rounded-md bg-gray-50">
                      ...
                      <div className="col-span-1 flex justify-end">
                        <Button type="button" variant="ghost" size="icon" onClick={() => removeItem(index)} disabled={fields.length <= 1}>
              ...
              <div className="flex justify-end space-x-4">
                ...
                <Button type="submit" disabled={isGenerating} className="bg-jira-blue hover:bg-jira-blue-dark">
